TASK: Audit and improve RESULTS.md for a deception honeypot cybersecurity portfolio project. The target reader is a hiring manager or senior security engineer evaluating real-world detection and analysis ability.

This is a DEFENSIVE SECURITY writeup review. The project is a legally-operated deception honeypot in Norway (detection and intelligence only, no offensive capability). The document analyzes attacker behavior captured by the sensor. Your job is to review it the way a senior blue-team reviewer would: check accuracy, completeness, structure, tone, technical correctness, and whether it would impress a hiring panel.

HARD CONSTRAINTS (non-negotiable):
- No em dashes or double hyphens anywhere. Use periods, commas, colons.
- American English spelling.
- Do not invent data. If something should be measured but is absent, say "not captured" rather than fabricating.
- Cite the source of each claim (DB query, session replay, command_exec, ThreatFox IOC, sha256).
- Preserve the existing section structure where it works, but flag any section that is weak, misleading, or missing.

AUDIT CHECKLIST (address each):
1. Data accuracy: do the numbers add up? Event counts, session counts, auth success rates, MITRE technique hits. Flag any internal inconsistency.
2. Technical accuracy: are the MITRE mappings correct? Is the kill chain reconstruction sound? Are the script analyses (clean.sh, setup.sh) accurate to what the commands show? Flag any wrong or unsupported claim.
3. Completeness: what would a hiring manager want to see that is missing? Think about: detection logic, false positives, honeypot limitations, defensive takeaways, comparison to real-world threat intel. Flag gaps.
4. Structure and flow: does the document tell a coherent story? Is the case study the right centerpiece? Should any section move, merge, or be cut?
5. Tone: is it technical, precise, evidence-backed, no hype? Flag any AI phrasing, vagueness, or overclaiming.
6. Opsec: does the document leak anything that should stay internal (honeypot markers, real VPS details, operator identity)? Flag any leak.
7. Reproducibility: could a reviewer reproduce the numbers from the described method? Flag any ambiguity.

OUTPUT: the full rewritten RESULTS.md, incorporating all fixes and improvements you identified. Markdown only. No commentary before or after, just the file content. If you keep a section unchanged, it stays. If you rewrite one, replace it in place. The result should be a polished, portfolio-ready document.

Here is the current RESULTS.md to audit:
...
> Data window: 2026-06-30 to 2026-07-08 (9 days). Figures below exclude the
...
Traffic breaks down as Cowrie SSH (the large majority), the Endlessh tarpit,
and a smaller set of interactions against the deception web app. Volume grew
8.8x over the four days from the first sample (3,620 events on 2026-07-04) to
the figures above.
...
session accounting is consistent. The 144 auth_success events (against 5,678
auth_attempts) put the overall brute-force success rate at roughly 2.5%.
...
in the Cowrie shell. The counts for T1049 and T1087 are inflated relative to a
real host because the honeypot logs every connection's attempt to enumerate
accounts and network state, not just successful ones.

## Credential guessing

The usernames attackers tried most were `root`, `admin`, and `support`, which
is consistent with the curated Cowrie `userdb` accepting the planted `admin`
decoy plus common weak `root` passwords. Successful logins (T1078) lead
attackers into the fake shell where the planted `/home/admin/.env` steers them
toward the deception web app.
...
The `support` account is notable: only 3 source IPs but 31 successes. A small
number of attackers specifically targeted `support` and succeeded repeatedly, a
real-world credential reuse pattern worth flagging.
...
- The x86_64 binary scores 37/64 on VirusTotal.

### Kill chain (reconstructed from session replay and command_exec)

1. T1110.001 Brute Force: hammered `root`, succeeded (T1078). 144 total
   auth_success across all IPs; root accounted for 71 of them from 40 distinct
   IPs.
2. T1082 System Information Discovery: ran `uname -s -m` to fingerprint
   architecture.
3. T1105 Ingress Tool Transfer: uploaded 6 files via SFTP to Cowrie, repeated 8
   times between 2026-07-04 and 2026-07-07.
4. Persistence: injected an SSH `authorized_keys` entry, then locked it with
   `chattr +ai` (immutable attribute). The key was labeled `rsa-key-20230629`,
   a known Outlaw signature.
5. T1059 Execution: ran
   `chmod +x clean.sh; sh clean.sh; rm -rf clean.sh; chmod +x setup.sh; sh setup.sh; rm -rf setup.sh; ...`
   then `setup.sh` executed the architecture-matched `redtail` binary.
...
2. `nohup` with a `curl`/`wget`/`/dev/tcp` fallback chain to download a
   `linux` binary to `/tmp`. The `/dev/tcp` fallback works even when `curl` and
   `wget` are absent, using the bash built-in TCP.
3. `head -c 3716336 > /tmp/...` writes exactly 3,716,336 bytes, a fixed-size
   payload.
4. Executes the binary with a large base64 blob as an argument (an encrypted
   C2 config or session key).
5. `echo 123456 > /tmp/.opass` drops a marker file.

These map to T1105 Ingress Tool Transfer and T1059 Command and Scripting
Interpreter. The honeypot logged the commands but did not execute them, which is
...
- 45.148.10.121: STUN/TURN relay to 141.101.90.1:3478 (Cloudflare), 30 events
  over several days. Attempting to use the box as a proxy for VoIP or NAT
  traversal.
- 176.53.159.196: DNS proxy to 1.1.1.1:53, 22 events. Using it as an open DNS
  resolver, possibly for DNS amplification.
- 64.89.162.38: HTTP proxy to itself on port 80. Checking whether it is an open
  proxy.

## Dominant noise source

101.201.76.235 (Alibaba Cloud, China): 21,302 events, 67% of all traffic. A
...
> Screenshots: add captures of the attack map, the MITRE heatmap, the Outlaw
> session replay (130.12.180.51), and top attacker profiles here (`docs/img/`)
> to make this section self-contained for readers who are not running the
> stack. A GIF of the 130.12.180.51 session replay would make the case study
> tangible.

## Reproducing these numbers

The `scripts/digest.sh` cron job writes a dated markdown summary (totals,
unique IPs, 24h deltas, top attackers by risk score, technique counts, recent
events) to `digests/`. This report is a curated snapshot of that data. The
event-type and technique counts above come from direct SQLite queries against
the intel store at `data/db/trap-house.db`.